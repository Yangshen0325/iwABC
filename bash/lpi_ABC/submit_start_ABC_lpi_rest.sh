...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --job-name=ABC_lpi_rest_all
#SBATCH --output=logs_lpi_90/ABC_lpi_rest_all-%j.log
#SBATCH --mem=3GB
...
if [ $# -ne 6 ]; then
  echo "Usage: sbatch submit_start_ABC_lpi_rest.sh <lac> <mu> <K> <gam> <laa> <ss_set>"
  exit 1
fi
...
REPS_PER_GROUP=${REPS_PER_GROUP:-90}  # test first N reps from each group
...
for (( g=0; g<NUM_GROUPS; g++ )); do
  ...
  for (( r=1; r<=REPS_PER_GROUP; r++ )); do
    ...
    sbatch ~/iwABC/bash/lpi_ABC/start_ABC_lpi_rest.sh \
           "${param_set}" \
           "${idparsopt_lac}" \
           "${idparsopt_mu}" \
           "${idparsopt_K}" \
