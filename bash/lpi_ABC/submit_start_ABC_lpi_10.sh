...
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --job-name=ABC_lpi_10_all
#SBATCH --output=logs_lpi_firstTen/ABC_lpi_10_all-%j.log
#SBATCH --mem=3GB
#SBATCH --partition=regular
#SBATCH --export=ALL   # <-- ensure env vars like START_OFFSET pass through

set -euo pipefail
mkdir -p logs_lpi_firstTen

# Arguments: lac mu K gam laa ss_set
if [ $# -ne 6 ]; then
  echo "Usage: sbatch submit_start_ABC_lpi_10.sh <lac> <mu> <K> <gam> <laa> <ss_set>"
  exit 1
fi
...
NUM_GROUPS=${NUM_GROUPS:-32}    # how many parameter combos
REPS_PER_GROUP=${REPS_PER_GROUP:-10}  # test first N reps from each group
START_OFFSET=${START_OFFSET:-0}    # how many reps to skip at start of each group
# --------------------------

echo "[LPI SUBMIT] GROUP_SIZE=${GROUP_SIZE} NUM_GROUPS=${NUM_GROUPS} START_OFFSET=${START_OFFSET} REPS_PER_GROUP=${REPS_PER_GROUP}"
...
for (( g=0; g<NUM_GROUPS; g++ )); do
  ...
  for (( r=1; r<=REPS_PER_GROUP; r++ )); do
    ...
    sbatch ~/iwABC/bash/lpi_ABC/start_ABC_lpi_10.sh \
           "${param_set}" \
           "${idparsopt_lac}" \
           "${idparsopt_mu}" \
           "${idparsopt_K}" \
