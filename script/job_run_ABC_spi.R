...
if (dir.exists(checkpoint_dir)) {
  ...
  pats <- sprintf("^chk_spi_set%04d_iter(\\d{2})\\.rds$", param_set)
  files <- list.files(checkpoint_dir, pattern = pats, full.names = TRUE)
  if (length(files) > 0) {
    # extract iteration numbers
    iters <- as.integer(sub(sprintf("^.*chk_spi_set%04d_iter(\\d{2})\\.rds$", param_set), "\\1", files))
...
iwABC::run_ABC_par_spi(
  ...
  # prior functions
  prior_generating_function = prior_gen_spi,
  prior_density_function = prior_dens_spi,
