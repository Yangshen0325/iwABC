...
#' prior density function for small-phylogeny islands
#'
#' @return Density of the given parameter set
#' @export
prior_dens_spi <- function(pars, idparsopt) {
  ...
  if(3 %in% idparsopt){
    dens_k <- stats::dunif(pars[3], 0, 200)
  } else {
    dens_k <- 1
  }
...
#' @return a vector of parameters
#' @export
prior_gen_spi <- function(pars,idparsopt){
