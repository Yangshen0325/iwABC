...
ABC_SMC_iw_par_spi <- function(
    ...
    checkpoint_dir       = "checkpoints_spi",
    resume_from          = 0,
  ...
  if (resume_from > 0 || !is.null(checkpoint_path)) {
    if (is.null(checkpoint_path)) {
      ...
      checkpoint_path <- file.path(checkpoint_dir, sprintf("chk_spi_set%04d_iter%02d.rds", param_set, resume_from))
    }
  ...
  for (i in i_start:num_iterations) {
    ...
    if (enable_checkpoint) {
      if (is.na(param_set)) {
        fn <- file.path(checkpoint_dir, sprintf("chk_spi_iter%02d.rds", i))
      } else {
        fn <- file.path(checkpoint_dir, sprintf("chk_spi_set%04d_iter%02d.rds", param_set, i))
      }
      checkpoint <- list(
        iter_completed  = i,
        ABC_list        = ABC_list,
